...
prepare_taxonomy <- function(data) {
  d <- data %>% dplyr::select(ID,all_of(tax)) %>% mutate(ID = paste0("ASV",ID)) %>% rename("Sequence" = "sequence")
  return(d)
}
...
prepare_taxonomy_sameregion <- function(x) {
  x %>%
    ...
    mutate(ID = paste0("ASV",sub("^[^_]*_", "", ID)))
  }
...
prepare_DA_data <- function(data,tax="Genus",meta=c("status","author")) {
   ...
   ## Extract metadata
   met <- lapply(data,function(x) sample_data(x) %>% data.frame() %>% dplyr::select(meta) %>% rownames_to_column("ID"))
